...
<body>
    ...
    <main>
        ...
        <!-- agendas area gantt chart and attendance-->
        <div class="agendas" id="chart">
          <img src="chart.png">
        </div>
        <div class="agendas" id="attendance">
        <h1>Class Attendance Management</h1>
        <form id="attendance-form" method="POST" action="save_attendance.php">
            <table>
                <thead>
                    <tr>
                        <th>Member</th>
                    ...
                    foreach ($members as $member) {
                        foreach ($dates as $date) {
                            echo "<tr>
                                <td>$member</td>
                                <td>$date</td>
                                <td><input type='time' name='time_in_{$member}_{$date}'></td>
                                <td><input type='time' name='time_out_{$member}_{$date}'></td>
                            </tr>";
                        }
                    }
                    ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        contactsLinks.forEach(link => {
            link.addEventListener('click', function(event) {
                ...
                toggleBlurOverlay('block'); //  blur  when modal opens
            });
        });
        ...
        window.addEventListener('click', function(event) {
            if (event.target === modal || blurOverlays.includes(event.target)) {
                modal.style.display = 'none';
                toggleBlurOverlay('none'); // Hide blur
            }
        });
